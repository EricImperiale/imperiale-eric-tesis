...
class EditRequest extends FormRequest
{
    ...
    {
        $guarantorId = $this->route('id');

        return [
            'name' => 'required|string|max:255',
            'last_name' => 'required|string|max:255',
            'dni' => 'required|digits:8|unique:guarantors,dni,' . $guarantorId,
            'cuit' => [
                'required',
                'digits:11',
                'unique:guarantors,cuit,' . $guarantorId,
                new VerifiyDniAndCuitMatch,
            ],
            'email' => 'required|email|unique:tenants,email,' . $guarantorId,
    ...
    public function messages()
    {
        return [
            ...
            'dni.unique' => 'Este DNI ya está registrado en otro garante.',
            'cuit.required' => 'El CUIT es obligatorio.',
            'cuit.digits' => 'El CUIT debe tener 11 dígitos.',
            'cuit.unique' => 'Este CUIT ya está registrado en otro garante.',
            'email.required' => 'El correo electrónico es obligatorio.',
            'email.email' => 'El correo electrónico debe ser válido.',
            'email.unique' => 'Este correo electrónico ya está registrado en otro garante.',
            ...
            'birth_date.before_or_equal' => 'La fecha de nacimiento debe ser una fecha anterior o igual a la actual.',
        ];
    }
}
